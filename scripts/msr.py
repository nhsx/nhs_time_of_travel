...
def main(city_or_county,filtered_df,target_address,network_type):
    ...
    new_df['Walking time (min)'] = (new_df['Distance in Miles'] / walking_speed * 60).round(0)
    new_df['Peak driving time (min)'] = (new_df['Distance in Miles'] / peak_driving_speed * 60).round(0)
    new_df['Off-peak driving time (min)'] = (new_df['Distance in Miles'] / off_peak_driving_speed * 60).round(0)
    new_df['Cycle time (min)'] = (new_df['Distance in Miles'] / cycle_speed * 60).round(0)
