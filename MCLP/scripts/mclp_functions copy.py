...
def get_target_nodes(G, list_of_target_addresses):
    ...
    return list_of_target_nodes


def sample_nodes(nodes, list_of_target_nodes):
    # generate the random sample of 100 samples by first removing the target node location from the dataframe
    nodes_df = nodes.copy()
    nodes_df = nodes_df[
        ~nodes_df.index.isin(list_of_target_nodes)
    ]  # get the dataframe of nodes that do not have any target nodes in them
...
def create_score(list_of_lengths):
    ...
    for l in list_of_lengths:
        deduction = (
            ((l / 1000) / 4.5) * 60
        ) * 5  # get the length in km divide by speed 4.5 km/h then divide by 60 to get time in minutes
        score = (
            score - deduction
...
def generate_target_scores(
    G, nodes_sample, list_of_target_nodes, list_of_target_addresses
):
    target_scores = {}
    site_names = []
    for i in range(len(list_of_target_nodes)):
        ...
        target_scores[site_name] = create_score(
            create_list_of_lengths(G, nodes_sample, list_of_target_nodes[i])
        )
        print(
            "The score for {}: {} is {}".format(
                site_name, site_address, target_scores[site_name]
            )
        )
...
def generate_route_maps(G, target_to_node_routes, site_names):
    route_maps = []
...
def mclp_main(region, list_of_target_addresses):
    ...
    list_of_target_nodes = get_target_nodes(G, list_of_target_addresses)
    nodes_sample = sample_nodes(nodes, list_of_target_nodes)
    site_names, target_scores = generate_target_scores(
        G, nodes_sample, list_of_target_nodes, list_of_target_addresses
    )
    target_to_node_routes = generate_msr(
        G, site_names, list_of_target_nodes, nodes_sample
    )
    route_maps = generate_route_maps(G, target_to_node_routes, site_names)
