...
def main(filtered_df,start_address,network_type, radius_miles):
    ...
    north = max([lat for (lat,long) in coords])+(0.01*(radius_miles/1.6)) # about a km extra
    south = min([lat for (lat,long) in coords])-(0.01*(radius_miles/1.6)) # about a km extra
    east = max([long for (lat,long) in coords])+(0.02*(radius_miles/1.6)) # about a km extra
    west = min([long for (lat,long) in coords])-(0.02*(radius_miles/1.6)) # about a km extra
